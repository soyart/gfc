...
simyn "Run test RSA on file dev/aes.key with keys dev/pub.pem and dev/pri.pem?"\
&& pub="files/pub.pem"\
&& pri="files/pri.pem"\
&& printf "Testing RSA encryption (ENV)\n"\
&& [[ -f $pub || -f $pri ]]\
&& RSA_PUB_KEY=$(< $pub) sh -c "${gfccmd} -rsa -i dev/aes.key -o /tmp/rsaOut"\
&& printf "Testing RSA decryption (ENV)\n"\
&& RSA_PRI_KEY=$(< $pri) sh -c "${gfccmd} -rsa -d -i /tmp/rsaOut -o /tmp/aes.key"\
&& printf "Testing equality\n"\
&& diff /tmp/aes.key dev/aes.key\
&& printf "✅ (ok) dev match\n"\
&& sh -c "${gfccmd} -rsa -k -pub $pub -i dev/aes.key -o /tmp/rsaOut"\
